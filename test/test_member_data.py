# coding: utf-8

"""
    Hosted Payment Controls API Documentation v1.5

    The API for Hosted Payment Controls. To access the swagger.json file open the following link: https://hpc.uat.freedompay.com/api/swagger/docs/v1.5

    The version of the OpenAPI document: v1.5
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from freedompay_client.models.member_data import MemberData  # noqa: E501

class TestMemberData(unittest.TestCase):
    """MemberData unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> MemberData:
        """Test MemberData
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `MemberData`
        """
        model = MemberData()  # noqa: E501
        if include_optional:
            return MemberData(
                enrollment_date = '',
                is_new_member = '',
                member_identification = '',
                member_identification_type = '',
                id = '',
                street1 = '',
                street2 = '',
                city = '',
                state = '',
                country = '',
                postal_code = '',
                phone_number = '',
                level = '',
                status = '',
                additional_data = '',
                additional_data_type = '',
                properties = [
                    freedompay_client.models.user_defined_field.UserDefinedField(
                        name = '', 
                        value = '', )
                    ]
            )
        else:
            return MemberData(
        )
        """

    def testMemberData(self):
        """Test MemberData"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
